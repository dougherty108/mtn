...
#Conduit
basemap <- get_tiles(junction_webmerc, provider = "CartoDB.Positron", crop = TRUE)

ggplot() +
  layer_spatial(basemap) +  # Basemap layer
  geom_sf(data = junction_webmerc, 
          aes(color = as.character(JUNCTION_SIZE)), 
          size = 1.0) +  # Your data
  geom_sf(
  ...
  labs(title = "Sterling Ranch--Junctions", color = "Junction Size") +
  theme_minimal()
